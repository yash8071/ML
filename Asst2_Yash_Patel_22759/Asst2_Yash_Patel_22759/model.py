...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.lora_proj = LoRALinear(config.n_embd , config.n_embd , config.LoRA_rank , alpha = 1)
        self.lora_attn = LoRALinear(config.n_embd , 3*config.n_embd , config.LoRA_rank , alpha = 1)
    ...
    def forward(self, x):
        ...
        attn_out = self.c_attn(x) + self.lora_attn(x)
        ...
        proj_out = self.c_proj(y) + self.lora_proj(y)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.lora_fc = LoRALinear(config.n_embd, 4 * config.n_embd, rank=config.LoRA_rank , alpha =1)
        self.lora_proj = LoRALinear(4 * config.n_embd, config.n_embd, rank=config.LoRA_rank , alpha = 1)
    ...
    def forward(self, x):
        ...
        x = self.gelu(self.c_fc(x) + self.lora_fc(x))
        x = self.dropout(self.c_proj(x)+ self.lora_proj(x))
        return x
...
class LoRALinear(nn.Module):
    def __init__(self, input_dim , output_dim , rank , alpha) -> None:
        super().__init__()
        sd = 1/(torch.sqrt(torch.tensor(rank)).float())
        self.X = torch.nn.Parameter(torch.randn(input_dim , rank) * sd)
        self.Y = torch.nn.Parameter(torch.zeros(rank , output_dim))
        self.alpha = alpha
    def forward(self, x):
        return x @ (self.X @ self.Y) * self.alpha
...
class DistilRNN(nn.Module):
    def __init__(self, in_dim , hidden_size , num_layers=1) -> None:
      super(DistilRNN , self).__init__()
      self.em = nn.Embedding(50304 , in_dim)
      self.rnn = nn.RNN(in_dim , hidden_size , num_layers , batch_first =True)
      self.nl = num_layers
      self.hs = hidden_size
      self.fc = nn.Linear(hidden_size , 2)

    def forward(self, x):
      h0 = torch.zeros(self.nl , x.size(0) , self.hs , dtype = torch.float32).to(x.device)
      x = self.em(x)
      out , v = self.rnn(x , h0)
      out = self.fc(out[:, -1])
      return out
